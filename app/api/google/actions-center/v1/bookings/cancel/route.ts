...
export async function POST(req: NextRequest) {
  ...
  const cancelReason = readString(body?.reason) ?? readString(body?.cancelReason) ?? readString(body?.cancel_reason) ?? null;
  ...
  logger.info({ requestId: auth.requestId, appointmentId: appointment.id }, "[actions-center] booking cancelled");
