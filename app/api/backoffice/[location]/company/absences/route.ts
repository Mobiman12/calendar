...
export async function PATCH(request: NextRequest, context: { params: Promise<{ location: string }> }) {
  ...
  metadataRecord.companyAbsences = payload.absences;
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Abwesenheiten konnten nicht gespeichert werden.";
  ...
  await logAuditEvent({
    ...
    entityType: "company_absences",
    entityId: locationRecord.id,
    diff: { absences: payload.absences },
    context: { source: "company_absences" },
