...
export default async function BookingManagePage({ params, searchParams }: PageProps) {
  const [{ tenant, token }, query] = await Promise.all([params, searchParams]);
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="w-full max-w-sm space-y-6">
            <div>
              ...
              {canCancel ? (
                <form
                  action={`/book/${encodeURIComponent(tenant)}/appointment/${encodeURIComponent(token)}/cancel`}
                  method="post"
...
function formatStaffName(staff: { displayName: string | null; firstName: string | null; lastName: string | null } | null) {
  ...
  const byDisplay = staff.displayName?.trim();
  if (byDisplay) return byDisplay;
  const combined = `${staff.firstName ?? ""} ${staff.lastName ?? ""}`.replace(/\s+/g, " ").trim();
