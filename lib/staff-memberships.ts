...
export async function supportsStaffMemberships(prisma: PrismaClient): Promise<boolean> {
  ...
  try {
    const result = await prisma.$queryRaw<Array<{ exists: boolean }>>(Prisma.sql`
      SELECT EXISTS (
        ...
        WHERE table_schema = current_schema()
          AND table_name = 'StaffLocationMembership'
      ) AS "exists"
    `);
    ...
    console.warn("[staff-memberships] detection failed, assuming unsupported", error);
